...
type Reward = { _id: string; key: string; name: string; description: string; type: string; pointsCost: number; tokensGranted: number; requiresSize: boolean; sizes?: string[] };
type Redemption = { pointsCost: number; status: string; voucherCode?: string; expiresAt?: string };
...
export default function RewardsPage() {
  ...
  useEffect(() => {
    Promise.all([apiFetch("/rewards"), apiFetch("/rewards/mine/history")])
      ...
      .catch((err) => setErrorMessage(err instanceof Error ? err.message : "Failed to load rewards."));
  }, []);
  ...
  const handleRedeem = async (reward: Reward) => {
    ...
    setModal("processing");
    try {
      const response = await apiFetch(`/rewards/${reward._id}/redeem`, { method: "POST", body: JSON.stringify(reward.requiresSize ? { size } : {}) });
      setRedemption(response.data ?? response);
      setModal("success");
    } catch (err) {
  ...
  return (
    <main className="min-h-screen bg-[#fbfbfb] flex flex-col items-center gap-6 lg:gap-8 px-4 md:px-16 py-6 lg:py-8">
      ...
      <div className="w-full max-w-[1312px] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {rewards.map((reward) => (
          <div key={reward._id} className="bg-white border border-[#f1e9e9] rounded-2xl overflow-hidden flex flex-col p-3.5 gap-6">
            <div className="relative h-[180px] rounded-2xl flex items-center justify-center overflow-hidden bg-[#eff7ff]">
              <span className="absolute top-3 right-3 bg-white text-xs font-bold text-[#f60] px-3 py-1.5 rounded-full">
                {reward.pointsCost} points
              </span>
              <span className="text-5xl">{reward.type === "airtime" ? "📱" : "🎁"}</span>
            </div>
            <div className="flex flex-col gap-4 px-4 flex-1">
              ...
              {reward.requiresSize && <select value={selectedSizes[reward._id] || ""} onChange={(e) => setSelectedSizes((current) => ({ ...current, [reward._id]: e.target.value }))} className="border border-[#e5e5e5] rounded-lg p-2 text-sm"><option value="">Select size</option>{(reward.sizes || []).map((size) => <option key={size} value={size}>{size}</option>)}</select>}
              <button
      ...
      {errorMessage && <p className="text-sm text-[#ff3b3b]">{errorMessage}</p>}
      <section className="w-full max-w-[1312px] bg-white border border-[#f2f4f7] rounded-2xl p-5"><p className="font-bold text-lg">Redemption History</p><p className="text-sm text-[#9f9f9f] mt-2">{history.length} redemption{history.length === 1 ? "" : "s"} recorded.</p></section>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50" onClick={() => setModal(null)}>
          ...
          >
            ...
            <div className="w-full bg-[#fdf3e0] rounded-xl p-4 flex items-center justify-center gap-3 text-xs font-bold">
              <span className="text-[#212121]">QUANT TOKEN</span>
              <span>⟶</span>
              <span className="text-[#212121]">AIRTIME VOUCHER</span>
            </div>
            ...
            >
              {modal === "success" && (redemption?.expiresAt ? `Expires on ${new Date(redemption.expiresAt).toLocaleDateString()}` : redemption?.status)}
              {modal === "failed" && "Failed"}
